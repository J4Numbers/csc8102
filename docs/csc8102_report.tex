...
\texttt{.8102} extention. A file which does not end with that extension is
...
\texttt{target/} folder. From there, the user can use the following commands
to encrypt and decrypt a file respectively: \texttt{java -jar [jarfile] -e
[filename]} and \texttt{java -jar [jarfile] -d [filename].8102}.
...
are trying to encrypt a file with the \texttt{.8102} extention, or if you are
...
Let us say that we wish to encrypt the file below that contains some
confidential data using this program.
