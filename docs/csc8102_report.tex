...
\author{J4Numbers, \{\url{j4numbers@gmail.com}\}}% <-this % stops a space
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\linewidth]{images/secret_decryption}
...
However, this is not really a very secure solution to any problem. While this
program does provide confidentiality to the data in that file at that point in
time, there is nothing stopping a malicious user from deleting that file
completely, at which point we will lose all records of who Daredevil is.
Additional controls could be put in place to protect the integrity of the
files, but there is not really much that could be done if an attacker has
physical access to the machine.

Another potential weakness of this program is that, if an attacker does get
access to this machine, then they are also able to also encrypt other files
with their own passwords that the user does not know and engineer a denial of
service attack against the user, denying them from accessing their files. Part
of the reason for this could be that there are no privileges required for this
program - it can be run by anyone on the machine to encrypt the files they have
access to.
